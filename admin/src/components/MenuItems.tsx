...
function MenuItems({children, title, icon = 'arrow-right', href, onClick }: MenuItemProps) {
  ...
  if (href) {
    ...
    if(children !== undefined) {
      return (
        <li className="list-none relative">
          <button onClick={toggleIsOpen} className="absolute z-10 top-2 left-0 fill-white p-0.5 w-4 h-4 cursor-pointer">
            <Icon name="chevron-right" size={10} className={"duration-200" + (isOpen ? " rotate-90": "")} />
          </button>
          ...
          <ul className={"sub-menu-items pl-4 overflow-clip h-0 " + (isOpen ? "h-full" : "pointer-events-none") }>
            {children}
          </ul>
        </li>
      );
